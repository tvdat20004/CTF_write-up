...
class CryptoSystem:
    ...
    def encrypt(self, data):
        if len(data) * 8 > 128:
            raise ValueError(f"Data too big! Data has {len(data)*8} bits, can only encrypt {128} bits")
    ...
    def decrypt(self, ct):
        ...
        for i in range(128-1, -1, -1):
            if self._prv_lst[i] <= inv:
                inv -= self._prv_lst[i]
                data |= (1 << (128 - 1 - i))
        return data.to_bytes(128//8, 'big')
...
def main():
    ...
    #     flag = f.read()
    flag = b'RS{fake_flag_231232193129}'
    enc_flag = sym_c.encrypt(pad(flag, AES.block_size))
    # print({'enc_flag': enc_flag.hex(), 'enc_key': enc_key.hex(), 'pub_key': c.pub_key, 'iv': iv.hex()})
    print(c._prv_r)
    print(c._prv_q)
