...
def main():
    des = DES(bytes.fromhex("0E329232EA6D0D73"))
    # des.round_keys = 
    assert des.encrypt(bytes.fromhex("8787878787878787")) == bytes.fromhex("0000000000000000")
    # assert des.decrypt(b'X\xa1K\xcek\x0c\xc4\x90') == b'0A1B2C3D'
    assert bytes.fromhex("8787878787878787") == des.decrypt(bytes.fromhex("0000000000000000"))
